...
den = f.Get("turnon/block/all/denominator")
x = range(5, 200, 5)
...
for threshold in x:
  ...
  num = f.Get("turnon/block/all/L1_GFEX_B{0:d}/numerator".format(threshold))
  gr = ROOT.TGraphAsymmErrors(num, den, str(threshold))
  gr.SetTitle('Turn-On Curve for Min-Bias for L1_GFEX_B{0:d}'.format(threshold))
  ...
  c.Modified()
  c.Update()
  c.SaveAs('turnon_L1_GFEX_B{0:d}.pdf'.format(threshold))
  c.Clear()
